...
% Run simulations
    ...
    %%  Generate matrices for storing the results of each pass through. 
    sample_size= 5000; %I^2/10;
    results= ones(sample_size,5);
    ...
    %generate a ten-percent random sample of nodes 
    ...
    for i =1:length(distance_shock)
        for j=1:sample_size
                ...
                results(j, 1)= t(sampled_nodes(j,1),sampled_nodes(j,2));
                results(j, 2)= results_init_eqbm.pi_ij(sampled_nodes(j,1),sampled_nodes(j,2));
                if mod( j , sample_size/10 )==0
                    display(j);
                end 
                if i==1 
                results(j, 3)= sim_eq.U_bar;
                elseif i==2 
                    results(j, 4)= sim_eq.U_bar;
                else 
                    results(j, 5)= sim_eq.U_bar; 
                end
        end
    end
save('../output/results.mat', 'results');
